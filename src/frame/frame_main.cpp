...
void key_subidscan_cb(epdgui_args_vector_t &args)
{
    Frame_Base *frame = EPDGUI_GetFrame("Frame_SubIdScan");
    if(frame == NULL)
    {
...
Frame_Main::Frame_Main(void): Frame_Base(false)
{
    ...
    *(_key[kKeyHome]->CanvasPressed()) = *(_key[kKeyHome]->CanvasNormal());
    ...
    _key[kKeySubIdScan]->CanvasNormal()->pushImage(0, 0, 92, 92, ImageResource_main_icon_wifi_92x92);
    *(_key[kKeySubIdScan]->CanvasPressed()) = *(_key[kKeySubIdScan]->CanvasNormal());
    _key[kKeySubIdScan]->CanvasPressed()->ReverseColor();
    _key[kKeySubIdScan]->AddArgs(EPDGUI_Button::EVENT_RELEASED, 0, (void*)(&_is_run));
    _key[kKeySubIdScan]->Bind(EPDGUI_Button::EVENT_RELEASED, key_subidscan_cb);
...
void Frame_Main::AppName(m5epd_update_mode_t mode)
{
    ...
    if(language == LANGUAGE_JA)
    {
        _names->drawString("Correlator", 20 + 46 + 3 * 136, 16);
    }
    else if(language == LANGUAGE_ZH)
    {
        _names->drawString("Correlator", 20 + 46 + 3 * 136, 16);
    ...
    {
        _names->drawString("Correlator", 20 + 46, 16);
    }
...
void Frame_Main::StatusBar(m5epd_update_mode_t mode)
{
    ...
    _bar->drawString("Hackthon Demo", 10, 27);
